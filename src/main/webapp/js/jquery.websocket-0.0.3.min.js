    ...
    .extend({
      WebSocket: function(url, protocol, options) {
        ...
        function _urlWsToHttp(url) {
          var protocol =
            (url.attr("protocol") === "wss") ? "https://" : "http://";
          ...
          ws[event] = function(e) {
            if (event == "onmessage") {
              ...
              for (pipe in ws.pipes) {
                $(ws.pipes[pipe]).trigger(e);
              }
            }
            (event == "send") ? e = _dispatchProtocol(e, "o", ws) : null;
            _proxy(event, e);
          };
          window.WebSocket.prototype.__defineSetter__(event, function(v) {
            _functionTable[event] = v;
          });
          window.WebSocket.prototype.__defineGetter__(event, function() {
        ...
        function _pipe(url) {
          return $.extend(_getWebSocketSkeleton(url, false), {
            packet: function(uid) {
              return {
                type: WS_ID,
                action: "message",
        ...
        function _WebSocket(url) {
          var _interval, _handler, _emulation =
            {
              send: function(data) {
                ...
                $.ajax({
                  ...
                  data: ((options.method == "POST" && options.arguments) ? $
                    .param($.extend(options.arguments, {
                      data: data
                    })) : null),
                  dataType: "text",
                  ...
                  success: function(data) {
                    $(_emulation).trigger(
                      new _MessageEvent(
                        _dispatchProtocol(data, "i", _emulation)));
                  },
                  error: function(xhr, data, errorThrown) {
                    success = false;
                    $(_emulation).trigger(_ErrorEvent(data));
                  }
                });
                return success;
              },
          ...
          function _poll() {
            $.ajax({
              ...
              dataType: "text",
              data: ((options.method == "POST" && options.arguments) ? $
                .param(options.arguments) : null),
              success: function(data) {
                $(_emulation).trigger(new _MessageEvent(data));
              },
              error: function(xhr, data, errorThrown) {
                success = false;
                $(_emulation).trigger(_ErrorEvent(data));
        ...
        function _getWebSocket(url) {
          ...
          return _extend(_getWebSocketSkeleton(url, isNative), ws);
        }
